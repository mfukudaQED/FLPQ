/**********************************************************************
  Set_Density_Grid_fuku_i.c:

     Set_Density_Grid_fuku_i.c is a subroutine to calculate a density matrix
     on grid by one-particle wave functions.

  Log of Set_Density_Grid_fuku_i.c:
...
double Set_Density_Grid_fuku_i(double *****CDM)
{
  ...
  size_Tmp_Den_Grid = 0;
  Tmp_Den_Grid = (double***)malloc(sizeof(double**)*(SpinP_switch_i+1)); 
  ...
  for (ID=0; ID<numprocs; ID++){
    Den_Snd_Grid_A2B[ID] = (double*)malloc(sizeof(double)*Num_Snd_Grid_A2B[ID]*(SpinP_switch_i+1));
    size_Den_Snd_Grid_A2B += Num_Snd_Grid_A2B[ID]*(SpinP_switch_i+1);
  }  

  size_Den_Rcv_Grid_A2B = 0;   
  Den_Rcv_Grid_A2B = (double**)malloc(sizeof(double*)*numprocs);
  for (ID=0; ID<numprocs; ID++){
    Den_Rcv_Grid_A2B[ID] = (double*)malloc(sizeof(double)*Num_Rcv_Grid_A2B[ID]*(SpinP_switch_i+1));
    size_Den_Rcv_Grid_A2B += Num_Rcv_Grid_A2B[ID]*(SpinP_switch_i+1);   
  }
  ...
  ******************************************************/
  ...
  for (Mc_AN=1; Mc_AN<=Matomnum; Mc_AN++){
    ...
    for (AN=0; AN<GridN_Atom[Gc_AN]; AN++){
      ...
      for (spin=0; spin<(SpinP_switch_i+1); spin++){
        Den_Snd_Grid_A2B[ID][Num_Snd_Grid_A2B[ID]*(SpinP_switch_i+1)+spin] = Tmp_Den_Grid[spin][Mc_AN][AN];
      }
  ...
  for (ID=1; ID<numprocs; ID++){
    ...
    if (Num_Snd_Grid_A2B[IDS]!=0){
      MPI_Isend( &Den_Snd_Grid_A2B[IDS][0], Num_Snd_Grid_A2B[IDS]*(SpinP_switch_i+1), 
	         MPI_DOUBLE, IDS, tag, mpi_comm_level1, &request_send[NN_S]);
      NN_S++;
    }

    if (Num_Rcv_Grid_A2B[IDR]!=0){
      MPI_Irecv( &Den_Rcv_Grid_A2B[IDR][0], Num_Rcv_Grid_A2B[IDR]*(SpinP_switch_i+1), 
  ...
  /* for myid */
  for (i=0; i<Num_Rcv_Grid_A2B[myid]*(SpinP_switch_i+1); i++){
  ...
  for (ID=0; ID<numprocs; ID++){
    ...
    for (LN=0; LN<Num_Rcv_Grid_A2B[ID]; LN++){
      ...
      if (Solver!=4 || (Solver==4 && atv_ijk[GRc][1]==0 )){
        ...
        if (SpinP_switch_i==0){
	      Density_Grid_B_i[0][BN] += Den_Rcv_Grid_A2B[ID][LN];
	      Density_Grid_B_i[1][BN] = Density_Grid_B_i[0][BN];
        }
        else if (SpinP_switch_i==1){
	      Density_Grid_B_i[0][BN] += Den_Rcv_Grid_A2B[ID][LN*2  ];
	      Density_Grid_B_i[1][BN] += Den_Rcv_Grid_A2B[ID][LN*2+1];
        }
...
/*********************************************
   ...
   Density_Grid_B_i[0][BN]　:  Im[n_{11}]
   Density_Grid_B_i[1][BN]　:  Im[n_{22}] 
*********************************************/
