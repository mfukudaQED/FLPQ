/**********************************************************************
  Set_ddDensity_Grid_fuku_i.c:

     Set_ddDensity_Grid_fuku_i.c is a subroutine to calculate a stress tensor density etc
     on grid by one-particle wave functions.

  Log of Set_ddDensity_Grid_fuku_i.c:

     23/Jun/2015  Released by M.Fukuda
...
double Set_ddDensity_Grid_i(double *****CDM)
{
  ...
  int Max_Size,My_Max;
  int size_Tmp_ddDen_Grid;
//  long size_Tmp_ddDen_Grid;
  ...
  double tmpxx,tmpyy,tmpzz;
  double tmpxy,tmpyz,tmpzx;
  double tmp0_0,tmp0_1,tmp0_2,tmp0_3;
  double sum_0,sum_1,sum_2,sum_3;
  double d1,d2,d3,cop,sip,sit,cot;
  double x,y,z,Cxyz[4];
  double TStime,TEtime;
  double ****Tmp_ddDen_Grid;
  double **Den_Snd_Grid_A2B;
  double **Den_Rcv_Grid_A2B;
  double *tmp_array,*tmpxx_array,*tmpyy_array,*tmpzz_array;
  double *tmpxy_array,*tmpyz_array,*tmpzx_array;
  double *tmp_array2,*tmpxx_array2,*tmpyy_array2,*tmpzz_array2;
  double *tmpxy_array2,*tmpyz_array2,*tmpzx_array2;
  ...
  SpinP_Grid_size = SpinP_switch_i;
 

  /* allocation of arrays */

  size_Tmp_ddDen_Grid = 0;
  Tmp_ddDen_Grid = (double****)malloc(sizeof(double***)*7); 
  for (k=0; k<7; k++){
     Tmp_ddDen_Grid[k] = (double***)malloc(sizeof(double**)*(SpinP_Grid_size+1)); 
     for (i=0; i<(SpinP_Grid_size+1); i++){
       Tmp_ddDen_Grid[k][i] = (double**)malloc(sizeof(double*)*(Matomnum+1)); 
       Tmp_ddDen_Grid[k][i][0] = (double*)malloc(sizeof(double)*1); 
       for (Mc_AN=1; Mc_AN<=Matomnum; Mc_AN++){
         Gc_AN = F_M2G[Mc_AN];
         Tmp_ddDen_Grid[k][i][Mc_AN] = (double*)malloc(sizeof(double)*GridN_Atom[Gc_AN]);
        
         /* AITUNE */
         for (Nc=0; Nc<GridN_Atom[Gc_AN]; Nc++){
            Tmp_ddDen_Grid[k][i][Mc_AN][Nc] = 0.0;
         }
         
         size_Tmp_ddDen_Grid += GridN_Atom[Gc_AN];
       }
     }
   }
  ...
  ****************************************************/
  ...
  if (Cnt_kind==0 && Cnt_switch==1){
	  ...
	  for (Mul0=0; Mul0<Spe_Num_CBasis[Cwan][L0]; Mul0++){
	    for (M0=0; M0<=2*L0; M0++){
	      ...
	      for (p=0; p<Spe_Specified_Num[Cwan][al]; p++){
	        ...
	        tmp0 += CntCoes[Mc_AN][al][p]*ddOrbs_Grid[0][Mc_AN][Nc][j];/* AITUNE */
	        tmpxx += CntCoes[Mc_AN][al][p]*ddOrbs_Grid[1][Mc_AN][Nc][j];/* AITUNE */
	        tmpyy += CntCoes[Mc_AN][al][p]*ddOrbs_Grid[2][Mc_AN][Nc][j];/* AITUNE */
	        tmpzz += CntCoes[Mc_AN][al][p]*ddOrbs_Grid[3][Mc_AN][Nc][j];/* AITUNE */
	        tmpxy += CntCoes[Mc_AN][al][p]*ddOrbs_Grid[4][Mc_AN][Nc][j];/* AITUNE */
	        tmpyz += CntCoes[Mc_AN][al][p]*ddOrbs_Grid[5][Mc_AN][Nc][j];/* AITUNE */
	        tmpzx += CntCoes[Mc_AN][al][p]*ddOrbs_Grid[6][Mc_AN][Nc][j];/* AITUNE */
	      }

	      CddOrbs_Grid[0][Mc_AN][al][Nc] = (Type_Orbs_Grid)tmp0;
	      CddOrbs_Grid[1][Mc_AN][al][Nc] = (Type_Orbs_Grid)tmpxx;
	      CddOrbs_Grid[2][Mc_AN][al][Nc] = (Type_Orbs_Grid)tmpyy;
	      CddOrbs_Grid[3][Mc_AN][al][Nc] = (Type_Orbs_Grid)tmpzz;
	      CddOrbs_Grid[4][Mc_AN][al][Nc] = (Type_Orbs_Grid)tmpxy;
	      CddOrbs_Grid[5][Mc_AN][al][Nc] = (Type_Orbs_Grid)tmpyz;
	      CddOrbs_Grid[6][Mc_AN][al][Nc] = (Type_Orbs_Grid)tmpzx;
	    }
	  }
        }
      }
    ...
    ***********************************************/
    ...
    tmpxx_array  = (double*)malloc(sizeof(double)*Max_Size);
    tmpyy_array  = (double*)malloc(sizeof(double)*Max_Size);
    tmpzz_array  = (double*)malloc(sizeof(double)*Max_Size);
    tmpxy_array  = (double*)malloc(sizeof(double)*Max_Size);
    tmpyz_array  = (double*)malloc(sizeof(double)*Max_Size);
    tmpzx_array  = (double*)malloc(sizeof(double)*Max_Size);
    tmp_array2   = (double*)malloc(sizeof(double)*Max_Size);
    tmpxx_array2 = (double*)malloc(sizeof(double)*Max_Size);
    tmpyy_array2 = (double*)malloc(sizeof(double)*Max_Size);
    tmpzz_array2 = (double*)malloc(sizeof(double)*Max_Size);
    tmpxy_array2 = (double*)malloc(sizeof(double)*Max_Size);
    tmpyz_array2 = (double*)malloc(sizeof(double)*Max_Size);
    tmpzx_array2 = (double*)malloc(sizeof(double)*Max_Size);
      ...
      if (ID!=0){
        ...
        if (F_Snd_Num[IDS]!=0){
          ...
          for (n=0; n<F_Snd_Num[IDS]; n++){
            ...
            for (i=0; i<NO0; i++){
              for (Nc=0; Nc<GridN_Atom[Gc_AN]; Nc++){
                tmp_array[k]   = CddOrbs_Grid[0][Mc_AN][i][Nc];
                tmpxx_array[k] = CddOrbs_Grid[1][Mc_AN][i][Nc];
                tmpyy_array[k] = CddOrbs_Grid[2][Mc_AN][i][Nc];
                tmpzz_array[k] = CddOrbs_Grid[3][Mc_AN][i][Nc];
                tmpxy_array[k] = CddOrbs_Grid[4][Mc_AN][i][Nc];
                tmpyz_array[k] = CddOrbs_Grid[5][Mc_AN][i][Nc];
                tmpzx_array[k] = CddOrbs_Grid[6][Mc_AN][i][Nc];
                k++;
              }          
            }
          } 
          ...
          MPI_Isend(&tmpxx_array[0], size1, MPI_DOUBLE, IDS,
                    tag, mpi_comm_level1, &request);
          MPI_Isend(&tmpyy_array[0], size1, MPI_DOUBLE, IDS,
                    tag, mpi_comm_level1, &request);
          MPI_Isend(&tmpzz_array[0], size1, MPI_DOUBLE, IDS,
                    tag, mpi_comm_level1, &request);
          MPI_Isend(&tmpxy_array[0], size1, MPI_DOUBLE, IDS,
                    tag, mpi_comm_level1, &request);
          MPI_Isend(&tmpyz_array[0], size1, MPI_DOUBLE, IDS,
                    tag, mpi_comm_level1, &request);
          MPI_Isend(&tmpzx_array[0], size1, MPI_DOUBLE, IDS,
                    tag, mpi_comm_level1, &request);
        }
        ...
        if (F_Rcv_Num[IDR]!=0){
          ...
          MPI_Recv(&tmpxx_array2[0], size2, MPI_DOUBLE, IDR, tag, mpi_comm_level1, &stat);
          MPI_Recv(&tmpyy_array2[0], size2, MPI_DOUBLE, IDR, tag, mpi_comm_level1, &stat);
          MPI_Recv(&tmpzz_array2[0], size2, MPI_DOUBLE, IDR, tag, mpi_comm_level1, &stat);
          MPI_Recv(&tmpxy_array2[0], size2, MPI_DOUBLE, IDR, tag, mpi_comm_level1, &stat);
          MPI_Recv(&tmpyz_array2[0], size2, MPI_DOUBLE, IDR, tag, mpi_comm_level1, &stat);
          MPI_Recv(&tmpzx_array2[0], size2, MPI_DOUBLE, IDR, tag, mpi_comm_level1, &stat);
          ...
          for (n=0; n<F_Rcv_Num[IDR]; n++){
            ...
            for (i=0; i<NO0; i++){
              for (Nc=0; Nc<GridN_Atom[Gc_AN]; Nc++){
                CddOrbs_Grid[0][Mc_AN][i][Nc] = tmp_array2[k];
                CddOrbs_Grid[1][Mc_AN][i][Nc] = tmpxx_array2[k];
                CddOrbs_Grid[2][Mc_AN][i][Nc] = tmpyy_array2[k];
                CddOrbs_Grid[3][Mc_AN][i][Nc] = tmpzz_array2[k];
                CddOrbs_Grid[4][Mc_AN][i][Nc] = tmpxy_array2[k];
                CddOrbs_Grid[5][Mc_AN][i][Nc] = tmpyz_array2[k];
                CddOrbs_Grid[6][Mc_AN][i][Nc] = tmpzx_array2[k];
                k++;
              }          
            }
          }
...
#pragma omp parallel shared(myid,G2ID,Orbs_Grid_FNAN,dOrbs_Grid_FNAN,ddOrbs_Grid_FNAN,List_YOUSO,Tmp_ddDen_Grid,Orbs_Grid,dOrbs_Grid,ddOrbs_Grid,COrbs_Grid,CdOrbs_Grid,CddOrbs_Grid,Cnt_switch,Cnt_kind,GListTAtoms2,GListTAtoms1,NumOLG,CDM,SpinP_Grid_size,SpinP_switch_i,WhatSpecies,ncn,F_G2M,natn,Spe_Total_CNO,M2G) private(OMPID,Nthrds,Mc_AN,h_AN,Stime_atom,Etime_atom,Gc_AN,Cwan,NO0,Gh_AN,Mh_AN,Rnh,Hwan,NO1,spin,i,j,k,tmp_CDM,Nog,Nc_0,Nc_1,Nc_2,Nc_3,Nh_0,Nh_1,Nh_2,Nh_3,orbs0_0,orbs0_1,orbs0_2,orbs0_3,orbs1_0,orbs1_1,orbs1_2,orbs1_3,dorbs0_0,dorbs0_1,dorbs0_2,dorbs0_3,dorbs1_0,dorbs1_1,dorbs1_2,dorbs1_3,sum_0,sum_1,sum_2,sum_3,tmp0_0,tmp0_1,tmp0_2,tmp0_3,Nc,Nh,orbs0,orbs1,dorbs0,dorbs1,sum,tmp0)
  {

    orbs0 = (double*)malloc(sizeof(double)*List_YOUSO[7]);
    orbs1 = (double*)malloc(sizeof(double)*List_YOUSO[7]);
    dorbs0 = (double**)malloc(sizeof(double*)*7);
    for (k=0; k<7; k++){
      dorbs0[k] = (double*)malloc(sizeof(double)*List_YOUSO[7]);
    }
    dorbs1 = (double**)malloc(sizeof(double*)*7);
    for (k=0; k<7; k++){
    ...
    dorbs0_0 = (double**)malloc(sizeof(double*)*7);
    dorbs0_1 = (double**)malloc(sizeof(double*)*7);
    dorbs0_2 = (double**)malloc(sizeof(double*)*7);
    dorbs0_3 = (double**)malloc(sizeof(double*)*7);
    for (k=0; k<7; k++){
    ...
    dorbs1_0 = (double**)malloc(sizeof(double*)*7);
    dorbs1_1 = (double**)malloc(sizeof(double*)*7);
    dorbs1_2 = (double**)malloc(sizeof(double*)*7);
    dorbs1_3 = (double**)malloc(sizeof(double*)*7);
    for (k=0; k<7; k++){
    ...
    tmp_CDM = (double***)malloc(sizeof(double**)*(SpinP_switch_i+1)); 
    for (i=0; i<(SpinP_switch_i+1); i++){
	...
	for (spin=0; spin<SpinP_switch_i+1; spin++){
	  for (i=0; i<NO0; i++){
	    for (j=0; j<NO1; j++){
...
#pragma omp for
	for (Nog=0; Nog<NumOLG[Mc_AN][h_AN]-3; Nog+=4){
	  ...
	  if (Cnt_kind==0 && Cnt_switch==1){
	    ...
	    for (k=0; k<7; k++){
	      for (i=0; i<NO0; i++){
	         dorbs0_0[k][i] = CddOrbs_Grid[k][Mc_AN][i][Nc_0];
	         dorbs0_1[k][i] = CddOrbs_Grid[k][Mc_AN][i][Nc_1];
	         dorbs0_2[k][i] = CddOrbs_Grid[k][Mc_AN][i][Nc_2];
	         dorbs0_3[k][i] = CddOrbs_Grid[k][Mc_AN][i][Nc_3];
	      }
	    }
	    for (j=0; j<NO1; j++){
	    ...
	    for (k=0; k<7; k++){
	      for (j=0; j<NO1; j++){
	         dorbs1_0[k][j] = CddOrbs_Grid[k][Mh_AN][j][Nh_0];
	         dorbs1_1[k][j] = CddOrbs_Grid[k][Mh_AN][j][Nh_1];
	         dorbs1_2[k][j] = CddOrbs_Grid[k][Mh_AN][j][Nh_2];
	         dorbs1_3[k][j] = CddOrbs_Grid[k][Mh_AN][j][Nh_3];
	      }
	    }
	  }
	  /* else if ! "now under the orbital optimization" */
	    ...
	    for (k=0; k<7; k++){
	      for (i=0; i<NO0; i++){
		      dorbs0_0[k][i] = ddOrbs_Grid[k][Mc_AN][Nc_0][i];
		      dorbs0_1[k][i] = ddOrbs_Grid[k][Mc_AN][Nc_1][i];
		      dorbs0_2[k][i] = ddOrbs_Grid[k][Mc_AN][Nc_2][i];
		      dorbs0_3[k][i] = ddOrbs_Grid[k][Mc_AN][Nc_3][i]; 
	      }
	    }
       ...
       if (G2ID[Gh_AN]==myid){
	      ...
	      for (k=0; k<7; k++){
	         for (j=0; j<NO1; j++){
		         dorbs1_0[k][j] = ddOrbs_Grid[k][Mh_AN][Nh_0][j];
		         dorbs1_1[k][j] = ddOrbs_Grid[k][Mh_AN][Nh_1][j];
		         dorbs1_2[k][j] = ddOrbs_Grid[k][Mh_AN][Nh_2][j];
		         dorbs1_3[k][j] = ddOrbs_Grid[k][Mh_AN][Nh_3][j]; 
	         }
	      }
	    }
       else{
	      ...
	      for (k=0; k<7; k++){
	         for (j=0; j<NO1; j++){
		         dorbs1_0[k][j] = ddOrbs_Grid_FNAN[k][Mc_AN][h_AN][Nog  ][j];
		         dorbs1_1[k][j] = ddOrbs_Grid_FNAN[k][Mc_AN][h_AN][Nog+1][j];
		         dorbs1_2[k][j] = ddOrbs_Grid_FNAN[k][Mc_AN][h_AN][Nog+2][j];
		         dorbs1_3[k][j] = ddOrbs_Grid_FNAN[k][Mc_AN][h_AN][Nog+3][j]; 
	         }
	      }
	    }
	  }
     ...
     for (k=0; k<7; k++){
	    for (spin=0; spin<=SpinP_switch_i; spin++){
	      ...
	      ai_tmpDGs[k][spin][Nc_0] += sum_0;
	      ai_tmpDGs[k][spin][Nc_1] += sum_1;
	      ai_tmpDGs[k][spin][Nc_2] += sum_2;
	      ai_tmpDGs[k][spin][Nc_3] += sum_3;
	      /*
	        Tmp_ddDen_Grid[k][spin][Mc_AN][Nc_0] += sum_0;
	        Tmp_ddDen_Grid[k][spin][Mc_AN][Nc_1] += sum_1;
	        Tmp_ddDen_Grid[k][spin][Mc_AN][Nc_2] += sum_2;
	        Tmp_ddDen_Grid[k][spin][Mc_AN][Nc_3] += sum_3;
	      */
       } /* spin */
	  } /* k */

     for (k=0; k<7; k++){
	    for (spin=2; spin<=SpinP_switch_i; spin++){
	      ...
	      ai_tmpDGs[k][spin+2][Nc_0] += sum_0;
	      ai_tmpDGs[k][spin+2][Nc_1] += sum_1;
	      ai_tmpDGs[k][spin+2][Nc_2] += sum_2;
	      ai_tmpDGs[k][spin+2][Nc_3] += sum_3;
	      /*
	        Tmp_ddDen_Grid[k][spin][Mc_AN][Nc_0] += sum_0;
	        Tmp_ddDen_Grid[k][spin][Mc_AN][Nc_1] += sum_1;
	        Tmp_ddDen_Grid[k][spin][Mc_AN][Nc_2] += sum_2;
...
#pragma omp for
	for (Nog = NumOLG[Mc_AN][h_AN] - (NumOLG[Mc_AN][h_AN] % 4); Nog<NumOLG[Mc_AN][h_AN]; Nog++){
	  ...
	  if (Cnt_kind==0 && Cnt_switch==1){
	    ...
	    for (k=0; k<7; k++){
	      for (i=0; i<NO0; i++){
	         dorbs0[k][i] = CddOrbs_Grid[k][Mc_AN][i][Nc];
	      }
	    }
	    for (j=0; j<NO1; j++){
	    ...
	    for (k=0; k<7; k++){
	      for (j=0; j<NO1; j++){
	         dorbs1[k][j] = CddOrbs_Grid[k][Mh_AN][j][Nh];
	      }
	    }
	  }
	  else{
	    ...
	    for (k=0; k<7; k++){
	      for (i=0; i<NO0; i++){
	         dorbs0[k][i] = ddOrbs_Grid[k][Mc_AN][Nc][i];
	      }
	    }
	    ...
	    if (G2ID[Gh_AN]==myid){
	      ...
	      for (k=0; k<7; k++){
	         for (j=0; j<NO1; j++){
		         dorbs1[k][j] = ddOrbs_Grid[k][Mh_AN][Nh][j];
	         }
	      }
	    }
	    else{
	      ...
	      for (k=0; k<7; k++){
	         for (j=0; j<NO1; j++){
		         dorbs1[k][j] = ddOrbs_Grid_FNAN[k][Mc_AN][h_AN][Nog][j];
	         }
	      }
	    }
	  }
	  ...
	  for (spin=0; spin<=SpinP_switch_i; spin++){
	    ...
	    for (k=0; k<7; k++){
	      ...
	      ai_tmpDGs[k][spin][Nc] += sum;
	      /*Tmp_ddDen_Grid[k][spin][Mc_AN][Nc] += sum;*/
       }
	  } /* spin */
...
#pragma omp flush(Tmp_ddDen_Grid)
  ...
  ******************************************************/
  ...
  /* copy Tmp_ddDen_Grid to Den_Snd_Grid_A2B */
    ...
    for (AN=0; AN<GridN_Atom[Gc_AN]; AN++){
      ...
      ID = (int)(n2D*(unsigned long long int)numprocs/N2D);
	   ...
	   for (k=0; k<7; k++){
         ...
         for (i=0; i<(SpinP_Grid_size+1); i++){
            Den_Snd_Grid_A2B[ID][Num_Snd_Grid_A2B[ID]*(SpinP_Grid_size+1)*7+itmp+i] = Tmp_ddDen_Grid[k][i][Mc_AN][AN];
         } /* i SpinP */
      } /* k */
      /*
      if (SpinP_switch_i==0){
   ...
   for (k=0; k<7; k++){
      for (spin=0; spin<(SpinP_Grid_size+1); spin++){
        for (BN=0; BN<My_NumGridB_AB; BN++){
          ddDensity_Grid_B_i[k][spin][BN] = 0.0;
        }
      }
   }
      ...
      if (Solver!=4 || (Solver==4 && atv_ijk[GRc][1]==0 )){
        ...
        if (SpinP_switch_i==0){
          for (k=0; k<7; k++){
            int itmp;
            itmp = k*(SpinP_Grid_size+1);
	    ddDensity_Grid_B_i[k][0][BN] += Den_Rcv_Grid_A2B[ID][LN*7+itmp+i];
	    ddDensity_Grid_B_i[k][1][BN] = ddDensity_Grid_B_i[k][0][BN];
          }
        }
        else if (SpinP_switch_i==1){
          for (k=0; k<7; k++){
            ...
            itmp = k*(SpinP_Grid_size+1);
            for (i=0; i<(SpinP_Grid_size+1); i++){
	            ddDensity_Grid_B_i[k][i][BN] += Den_Rcv_Grid_A2B[ID][LN*(SpinP_Grid_size+1)*7+itmp+i];
            }
          }
        }
...
/*******************************************************
 double ***ddDensity_Grid_B; 
  ...
  size: ddDensity_Grid[7][6][My_NumGridB_AB]
       ...
       ddDensity_Grid_B_i[k][0][BN]　:  Im[nd_{11}^k]
       ddDensity_Grid_B_i[k][1][BN]　:  Im[nd_{22}^k] 
*********************************************/
  ...
  for (k=0; k<7; k++){
     for (i=0; i<(SpinP_Grid_size+1); i++){
       for (Mc_AN=0; Mc_AN<=Matomnum; Mc_AN++){
         free(Tmp_ddDen_Grid[k][i][Mc_AN]);
       }
       free(Tmp_ddDen_Grid[k][i]);
     }
     free(Tmp_ddDen_Grid[k]);
  }
  free(Tmp_ddDen_Grid);
