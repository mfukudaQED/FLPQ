/**********************************************************************
  Set_dDensity_Grid_fuku_i.c:

     Set_dDensity_Grid_fuku_i.c is a subroutine to calculate local physical quantities
     on grid by one-particle wave functions.

  Log of Set_dDensity_Grid_fuku_i.c:
...
//    }
//  else{
//      printf("Set_dDensity_Grid is unavailable for SpinP_switch_i<=2.\n");
//      exit(1);
//    }
  ...
  Tmp_dDen_Grid = (double****)malloc(sizeof(double***)*3); 
  for (k=0; k<3; k++){
  ...
  for (ID=0; ID<numprocs; ID++){
    Den_Snd_Grid_A2B[ID] = (double*)malloc(sizeof(double)*Num_Snd_Grid_A2B[ID]*(SpinP_Grid_size+1)*3);
    size_Den_Snd_Grid_A2B += Num_Snd_Grid_A2B[ID]*(SpinP_Grid_size+1)*3;
  }  

  size_Den_Rcv_Grid_A2B = 0;   
  Den_Rcv_Grid_A2B = (double**)malloc(sizeof(double*)*numprocs);
  for (ID=0; ID<numprocs; ID++){
    Den_Rcv_Grid_A2B[ID] = (double*)malloc(sizeof(double)*Num_Rcv_Grid_A2B[ID]*(SpinP_Grid_size+1)*3);
    size_Den_Rcv_Grid_A2B += Num_Rcv_Grid_A2B[ID]*(SpinP_Grid_size+1)*3;   
  }
  ...
  ******************************************************/
    ...
    for (AN=0; AN<GridN_Atom[Gc_AN]; AN++){
      ...
      ID = (int)(n2D*(unsigned long long int)numprocs/N2D);
	   ...
	   for (k=0; k<3; k++){
         ...
         for (i=0; i<(SpinP_Grid_size+1); i++){
            Den_Snd_Grid_A2B[ID][Num_Snd_Grid_A2B[ID]*(SpinP_Grid_size+1)*3+itmp+i] = Tmp_dDen_Grid[k][i][Mc_AN][AN];
         } /* i SponP */
      } /* k */
      /*
      if (SpinP_switch_i==0){
  ...
  for (ID=1; ID<numprocs; ID++){
    ...
    if (Num_Snd_Grid_A2B[IDS]!=0){
      MPI_Isend( &Den_Snd_Grid_A2B[IDS][0], Num_Snd_Grid_A2B[IDS]*(SpinP_Grid_size+1)*3, 
	         MPI_DOUBLE, IDS, tag, mpi_comm_level1, &request_send[NN_S]);
      NN_S++;
    }

    if (Num_Rcv_Grid_A2B[IDR]!=0){
      MPI_Irecv( &Den_Rcv_Grid_A2B[IDR][0], Num_Rcv_Grid_A2B[IDR]*(SpinP_Grid_size+1)*3, 
  ...
  /* for myid */
  for (i=0; i<Num_Rcv_Grid_A2B[myid]*(SpinP_Grid_size+1)*3; i++){
   ...
   for (k=0; k<3; k++){
      for (spin=0; spin<(SpinP_Grid_size+1); spin++){
        for (BN=0; BN<My_NumGridB_AB; BN++){
          dDensity_Grid_B_i[k][spin][BN] = 0.0;
        }
      }
   }
      ...
      if (Solver!=4 || (Solver==4 && atv_ijk[GRc][1]==0 )){
        if (SpinP_switch_i==0){
          for (k=0; k<3; k++){
             int itmp;
             itmp = k*(SpinP_Grid_size+1);
	     dDensity_Grid_B_i[k][0][BN] += Den_Rcv_Grid_A2B[ID][LN*3+itmp+i];
	     dDensity_Grid_B_i[k][1][BN] = dDensity_Grid_B_i[k][0][BN];
          }
        }
        else if (SpinP_switch_i==1){
          for (k=0; k<3; k++){
             ...
             itmp = k*(SpinP_Grid_size+1);
             for (i=0; i<(SpinP_Grid_size+1); i++){
	             dDensity_Grid_B_i[k][i][BN] += Den_Rcv_Grid_A2B[ID][LN*(SpinP_Grid_size+1)*3+itmp+i];
             }
          }
        }
