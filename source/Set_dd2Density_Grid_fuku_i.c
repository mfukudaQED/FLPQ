/**********************************************************************
  Set_dd2Density_Grid_fuku_i.c:

     Set_dd2Density_Grid_fuku_i.c is a subroutine to calculate local physical quantities 
     on grid by one-particle wave functions.

  Log of Set_dd2Density_Grid_fuku_i.c:
   ...
   for (k=0; k<7; k++){
      for (spin=0; spin<(SpinP_Grid_size+1); spin++){
        for (BN=0; BN<My_NumGridB_AB; BN++){
          dd2Density_Grid_B_i[k][spin][BN] = 0.0;
        }
      }
   }
      ...
      if (Solver!=4 || (Solver==4 && atv_ijk[GRc][1]==0 )){
        ...
        if (SpinP_switch_i==0){
          for (k=0; k<7; k++){
            int itmp;
            itmp = k*(SpinP_Grid_size+1);
	    dd2Density_Grid_B_i[k][0][BN] += Den_Rcv_Grid_A2B[ID][LN*(SpinP_Grid_size+1)*7+itmp+i];
	    dd2Density_Grid_B_i[k][1][BN] = dd2Density_Grid_B_i[k][0][BN];
          }
        }
        else if (SpinP_switch_i==1){
          for (k=0; k<7; k++){
            ...
            itmp = k*(SpinP_Grid_size+1);
            for (i=0; i<(SpinP_Grid_size+1); i++){
	            dd2Density_Grid_B_i[k][i][BN] += Den_Rcv_Grid_A2B[ID][LN*(SpinP_Grid_size+1)*7+itmp+i];
            }
          }
        }
...
/*******************************************************
 double ***dd2Density_Grid_B; 
  ...
  size: dd2Density_Grid[7][6][My_NumGridB_AB]
       ...
       dd2Density_Grid_B_i[k][0][BN]　:  Im[nd_{11}^k]
       dd2Density_Grid_B_i[k][1][BN]　:  Im[nd_{22}^k] 
*******************************************************/
