/**********************************************************************
  Set_ddDensity_Grid_fuku.c:

     Set_ddDensity_Grid_fuku.c is a subroutine to calculate a stress tensor density etc
     on grid by one-particle wave functions.

  Log of Set_ddDensity_Grid_fuku.c:

     24/Jan/2015  Released by M.Fukuda
	  ...
	  if (Cnt_kind==0 && Cnt_switch==1){
	    ...
	    for (k=0; k<7; k++){
	      for (i=0; i<NO0; i++){
	         ...
	         //dorbs0[k][i] = ddOrbs_Grid[k][Mc_AN][i][Nc];
	      }
	    }
   ...
   for (k=0; k<7; k++){
      for (spin=0; spin<(SpinP_Grid_size+1); spin++){
        for (BN=0; BN<My_NumGridB_AB; BN++){
          ddDensity_Grid_B[k][spin][BN] = 0.0;
        }
      }
   }
    ...
    for (LN=0; LN<Num_Rcv_Grid_A2B[ID]; LN++){
      ...
      if (Solver!=4 || (Solver==4 && atv_ijk[GRc][1]==0 )){
	      ...
	      for (k=0; k<7; k++){
            ...
            itmp = k*(SpinP_Grid_size+1);
            for (i=0; i<(SpinP_Grid_size+1); i++){
	            ddDensity_Grid_B[k][i][BN] += Den_Rcv_Grid_A2B[ID][LN*(SpinP_Grid_size+1)*7+itmp+i];
            }
         }
...
/*******************************************************
 double ***ddDensity_Grid_B; 
  ...
  size: ddDensity_Grid[7][6][My_NumGridB_AB]
       ...
       ddDensity_Grid_B[k][0][BN]　:  Re[nd_{11}^k]
       ddDensity_Grid_B[k][1][BN]　:  Re[nd_{22}^k] 
       ddDensity_Grid_B[k][2][BN]　:  Re[nd_{12}^k]
       ddDensity_Grid_B[k][3][BN]　:  Im[nd_{12}^k]
       ddDensity_Grid_B[k][4][BN]　:  Re[nd_{21}^k]
       ddDensity_Grid_B[k][5][BN]　: -Im[nd_{21}^k]
*********************************************/
//  /******************************************************
