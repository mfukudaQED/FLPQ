/**********************************************************************
  Set_dDensity_Grid_fuku.c:

     Set_dDensity_Grid_fuku.c is a subroutine to calculate local physical quantities
     on grid by one-particle wave functions.

  Log of Set_dDensity_Grid_fuku.c:
          ...
          if (Cnt_kind==0 && Cnt_switch==1){
            ...
            for (k=0; k<3; k++){
              for (i=0; i<NO0; i++){
                 ...
                 //dorbs0[k][i] = dOrbs_Grid[k][Mc_AN][i][Nc];
              }
            }
            ...
            if (G2ID[Gh_AN]==myid){
              ...
              for (k=0; k<3; k++){
                 for (j=0; j<NO1; j++){
//printf("k, Mh_AN, j, Nh = %d, %d, %d, %d\n",k, Mh_AN, j, Nh );fflush(stdout);
  ...
  for (ID=0; ID<numprocs; ID++){
    ...
    for (LN=0; LN<Num_Rcv_Grid_A2B[ID]; LN++){
      ...
      if (Solver!=4 || (Solver==4 && atv_ijk[GRc][1]==0 )){
            ...
            itmp = k*(SpinP_Grid_size+1);
            for (i=0; i<(SpinP_Grid_size+1); i++){
                    dDensity_Grid_B[k][i][BN] += Den_Rcv_Grid_A2B[ID][LN*(SpinP_Grid_size+1)*3+itmp+i];
            }
         }
